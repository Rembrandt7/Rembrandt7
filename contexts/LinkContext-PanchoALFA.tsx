...
const INITIAL_CONFIG: AppConfig = {
  ...
  linksBar: [
    ...
    {
        id: '3',
        href: "https://www.notion.so/302b0c914ade80b9ae47eb365541c7dc?v=302b0c914ade80bc8b76000ca2c99a9",
        name: "Notion",
  ...
  usefulTools: [
    ...
    },
    {
        ...
        iconSvg: '<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M4 19h16l-3-12h-10l-3 12zM5.5 19v2a1 1 0 001 1h11a1 1 0 001-1v-2" strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}/><path d="M12 7v12M9 13h6" strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}/></svg>',
        items: [
      ...
      try {
        ...
        if (finalConfig.usefulTools && Array.isArray(finalConfig.usefulTools)) {
          ...
          if (section3D) {
            ...
            if (!section3D.items.find((l: any) => l.name === 'Thingiverse')) {
              const tv = INITIAL_CONFIG.usefulTools.find(s => s.id === 'ut-5')?.items.find(l => l.name === 'Thingiverse');
              if (tv) section3D.items.push(tv);
            }
            if (!section3D.items.find((l: any) => l.name === 'Cults3D')) {
              const cults = INITIAL_CONFIG.usefulTools.find(s => s.id === 'ut-5')?.items.find(l => l.name === 'Cults3D');
              if (cults) section3D.items.push(cults);
            }
            if (!section3D.items.find((l: any) => l.name === 'MyMiniFactory')) {
              const mmf = INITIAL_CONFIG.usefulTools.find(s => s.id === 'ut-5')?.items.find(l => l.name === 'MyMiniFactory');
              if (mmf) section3D.items.push(mmf);
            }
